...
function iStemPast(verb) {
  ...
  const result = {};
  for (const p of PERSONS) {
    result[p] = conjugateIstemPastGeneral(verb, p);
  }
  return result;
}

function conjugateIstemPastGeneral(verb, person) {
  const is3 = person === 'sg3';
  ...
  if (vowelCount === 1 && verb.length >= 3) {
    const v = vowels[0];
    const nv = is3 ? VOWEL_CHANGE_3[v] : VOWEL_CHANGE[v];
    let mutC = MUTATIONS[modVerb.slice(0, 2)] || MUTATIONS[modVerb[0]] || modVerb[0];
 ...
 */
export function conjugate(sindarin, verbClass) {
